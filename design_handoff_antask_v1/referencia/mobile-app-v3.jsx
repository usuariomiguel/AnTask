...
function MPrio({ p, t }) {
  ...
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11.5, fontWeight: 650, fontFamily: t.fontMono, padding: '2px 8px', borderRadius: t.rTag, color: c.fg, background: c.bg }}><span style={{ fontSize: 8 }}>{c.mark}</span>{c.label}</span>;
}
...
function MLabel({ label, lk, t }) {
  ...
  return <span style={{ fontSize: 11.5, fontWeight: 600, fontFamily: t.monoMeta ? t.fontMono : t.fontUI, padding: '2px 8px', borderRadius: t.rTag, color: fg, background: c.bg }}>{label}</span>;
}
...
function MTaskCard({ task, t, tone, onToggle, onOpen, onMoveToday, hint, onDelete, rowStyle = 'tarjetas', idx = 0, last = false }) {
  ...
  const barColor = task.prio ? window.ANTASK_PRIO[task.prio].bar : t.accent;
  ...
  const rowStyleObj = {
    ...
    borderRadius: radius,
    padding: '13px 15px 13px 16px', position: 'relative', overflow: 'hidden',
  ...
  return (
    ...
    <div onClick={() => onOpen && onOpen(task.id)} style={rowStyleObj}>
      <span style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 4, background: task.prio ? barColor : 'transparent' }} />
      ...
      {dt && (
        tone === 'overdue' && !done
          ? <span onClick={(e) => { e.stopPropagation(); onMoveToday(task.id); }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11.5, fontWeight: 700, fontFamily: t.fontMono, whiteSpace: 'nowrap', flexShrink: 0, cursor: 'pointer',
...
function MSectionHead({ t, icon, label, count, tone }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9, margin: '22px 4px 11px' }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, fontFamily: t.fontMono, padding: '1px 7px', borderRadius: 999, background: tone === 'overdue' ? 'rgba(176,71,63,0.13)' : t.tintBg, color: c }}>{count}</span>
    </div>);
}
...
function MProgressRing({ t, done, total }) {
  ...
  const SZ = 40, R = 16, C = 2 * Math.PI * R;
...
function MHoyAdd({ t, onAdd }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: t.card, border: `1px solid ${active ? t.accent : t.border}`, borderRadius: mR(t.rInput, 2, 10), padding: '12px 14px', marginTop: 9, boxShadow: active ? `0 0 0 3px ${t.tintBg}` : '0 1px 2px rgba(40,30,15,0.05)', transition: 'border-color .12s, box-shadow .15s' }}>
      ...
      {v.trim() && <span onClick={submit} onMouseDown={(e) => e.preventDefault()} role="button" style={{ padding: '6px 13px', borderRadius: 999, background: t.accent, color: t.accentOn, fontSize: 12.5, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>Añadir</span>}
    </div>);
}
      ...
      {consultas.length > 0 && <React.Fragment>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          ...
          <div key={i} onClick={() => openNote(it.noteId)} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, background: t.card, border: `1px solid ${t.border}`, borderRadius: t.rInput + 2, padding: '13px 15px', position: 'relative', overflow: 'hidden', boxShadow: '0 1px 2px rgba(40,30,15,0.05)' }}>
            <span style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 4, background: types.consulta.fg }} />
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, background: t.canvas, borderRadius: '22px 22px 0 0', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', animation: 'mSheet .26s cubic-bezier(0.22,1,0.36,1) both', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '0 12px 8px' }}>
          {sections.map((sec, si) => (
            <div key={si}>
              {sec.heading &&
                <div style={{ fontSize: 11.5, fontWeight: 700, fontFamily: t.fontMono, letterSpacing: '0.07em', textTransform: 'uppercase', color: t.ink3, margin: si === 0 ? '6px 6px 4px' : '14px 6px 4px' }}>{sec.heading}</div>}
              {sec.options.map((o) => (
    ...
    <div>
      ...
      {done.length > 0 && <React.Fragment>
        <div onClick={() => setShowDone((v) => !v)} style={{ display: 'flex', alignItems: 'center', gap: 9, margin: '22px 4px 9px', cursor: 'pointer', userSelect: 'none' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, fontFamily: t.fontMono, padding: '1px 7px', borderRadius: 999, background: t.tintBg, color: t.ink2 }}>{done.length}</span>
          <span style={{ flex: 1 }} />
      ...
      {ordered.map((n) => {
        ...
        return (
          <div key={n.id} onClick={() => openNote(n.id)} style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: mR(t.rInput, 2, 8), padding: '14px 15px', boxShadow: '0 1px 2px rgba(40,30,15,0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 650, fontFamily: t.fontMono, padding: '3px 8px 3px 7px', borderRadius: t.rTag, color: m.fg, background: m.bg }}><MIco name={m.icon} size={11} sw={2} />{m.label}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '8px 0' }}>
              ...
              {!editing && <span onClick={() => setEditLine(l.id)}
                style={{ flexShrink: 0, marginTop: 3, display: 'inline-flex', alignItems: 'center', gap: 3, padding: '3px 7px', borderRadius: t.rTag, cursor: 'pointer',
                  fontSize: 10.5, fontWeight: 700, fontFamily: t.fontMono, color: m.fg, background: m.bg, opacity: 0.75, transition: 'opacity .12s' }}>
    ...
    <div onClick={locked ? undefined : onClick} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '14px 16px', cursor: locked ? 'default' : 'pointer', background: t.card, borderBottom: last ? 'none' : `1px solid ${t.border}`, opacity: locked ? 0.5 : 1 }}>
      ...
      {locked
        ...
        : <React.Fragment>
            ...
            {onEdit
              ? <span onClick={(e) => { e.stopPropagation(); onEdit(); }} aria-label="Editar lista" style={{ display: 'grid', placeItems: 'center', width: 30, height: 30, marginRight: -6, borderRadius: 8, color: t.ink3, cursor: 'pointer' }}><MIco name="settings" size={17} /></span>
    ...
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, background: t.canvas, borderRadius: '22px 22px 0 0', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', animation: 'mSheet .26s cubic-bezier(0.22,1,0.36,1) both', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 16px 14px' }}>
          <span onClick={onClose} style={{ fontSize: 14.5, fontWeight: 600, color: t.ink2, padding: '11px 16px', cursor: 'pointer' }}>Cancelar</span>
          <span style={{ flex: 1 }} />
          <span onClick={() => valid && onSave({ id: initial ? initial.id : 'l' + Date.now(), name: name.trim(), color, icon, count: initial ? initial.count : null })}
    ...
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, background: t.canvas, borderRadius: '22px 22px 0 0', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', paddingBottom: 0, animation: 'mSheet .26s cubic-bezier(0.22,1,0.36,1) both', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 16px 14px' }}>
          <span onClick={onClose} style={{ fontSize: 14.5, fontWeight: 600, color: t.ink2, padding: '11px 16px', cursor: 'pointer' }}>Cancelar</span>
          <span style={{ flex: 1 }} />
          <span onClick={() => { if (text.trim() && onCreate) onCreate({ type, text }); }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 14.5, fontWeight: 700, color: text.trim() ? t.accentOn : t.ink3, background: text.trim() ? t.accent : t.border, padding: '11px 22px', borderRadius: 999, cursor: text.trim() ? 'pointer' : 'default', transition: 'all .12s' }}>
...
function MTabBar({ t, screen, go, onCapture, v1, addBtn }) {
  ...
  const AddFab = ({ size = 50, inline = true }) => (
    <span onClick={onCapture} style={{ display: 'grid', placeItems: 'center', cursor: 'pointer', flexShrink: 0, width: size, height: size, margin: inline ? '-3px 0' : 0,
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, flexShrink: 0, padding: '8px 14px 22px', background: 'transparent', zIndex: 30, pointerEvents: 'none', display: 'flex', alignItems: 'center', gap: 11 }}>
      <div style={{ position: 'relative', flex: 1, padding: '8px 12px', borderRadius: 28, pointerEvents: 'auto',
        ...
        const Tab = (tb) => {
          ...
          if (tb.id === 'menu') return (
            <span key="menu" onClick={() => go('menu')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', minWidth: 52, padding: '2px 0' }}>
              ...
              <span style={{ fontSize: 10.5, fontWeight: on ? 700 : 600, letterSpacing: '0.01em', color: on ? t.accentInk : t.ink2 }}>{tb.label}</span>
            </span>);
          const locked = tb.id === 'notas';
          return (
            <span key={tb.id} onClick={locked ? undefined : () => go(tb.id)} style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: locked ? 'default' : 'pointer', color: on ? t.accentInk : t.ink3, opacity: locked ? 0.42 : 1, transition: 'color .15s', minWidth: 52, padding: '2px 0' }}>
              ...
              <span style={{ fontSize: 10.5, fontWeight: on ? 700 : 550, letterSpacing: '0.01em' }}>{tb.label}</span>
            </span>);
        };
  ...
  return (
    <div style={{ paddingBottom: 8 }}>
      ...
      <div style={{ marginTop: 16, padding: '14px 16px', borderRadius: t.rInput + 4, background: t.card, border: `1px solid ${t.border}` }}>
        <div style={{ marginBottom: 6, fontSize: 11, fontWeight: 700, fontFamily: t.fontMono, letterSpacing: '0.05em', textTransform: 'uppercase', color: t.ink2 }}>Plan gratis</div>
...
function MNoteOptions({ t, note, onTogglePin, onRename, onDuplicate, onDelete, onClose }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <div onClick={onClose} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(20,14,6,0.42)', animation: 'mFade .2s ease both' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, background: t.canvas, borderRadius: '22px 22px 0 0', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', paddingBottom: 22, animation: 'mSheet .26s cubic-bezier(0.22,1,0.36,1) both', overflow: 'hidden' }}>
        <div style={{ width: 38, height: 5, borderRadius: 999, background: t.border, margin: '9px auto 2px' }} />
        <div style={{ padding: '10px 20px 12px' }}>
          <div style={{ fontSize: 11.5, fontWeight: 700, fontFamily: t.fontMono, letterSpacing: '0.05em', textTransform: 'uppercase', color: t.ink3 }}>Opciones de nota</div>
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, background: t.canvas, borderRadius: '22px 22px 0 0', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', paddingBottom: 22, animation: 'mSheet .26s cubic-bezier(0.22,1,0.36,1) both', overflow: 'hidden' }}>
        <div style={{ width: 38, height: 5, borderRadius: 999, background: t.border, margin: '9px auto 2px' }} />
        <div style={{ padding: '10px 20px 12px' }}>
          <div style={{ fontSize: 11.5, fontWeight: 700, fontFamily: t.fontMono, letterSpacing: '0.05em', textTransform: 'uppercase', color: t.ink3 }}>Opciones de tarea</div>
...
function MToast({ t, label, onUndo }) {
  ...
  return (
    <div style={{ position: 'absolute', left: 14, right: 14, bottom: 92, zIndex: 60, display: 'flex', alignItems: 'center', gap: 12,
      ...
      <span onClick={onUndo} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onUndo(); } }}
        style={{ fontSize: 14.5, fontWeight: 750, color: t.accent, cursor: 'pointer', padding: '6px 12px', borderRadius: 9, whiteSpace: 'nowrap' }}>Deshacer</span>
    </div>);
}
...
function MobileApp({ theme: t, tw, setTweak, v1 }) {
  ...
  const [tasks, setTasks] = React.useState(() => (window.ANTASK_DATED || []).map((x) => ({ ...x })));
  const [linesByNote, setLines] = React.useState(() => window.buildNoteLines());
  const [notes, setNotes] = React.useState(() => (window.NOTES_DATA || []).map((n) => ({ ...n })));
  ...
  const [lists, setLists] = React.useState(M_INIT_LISTS);
  const [listSheet, setListSheet] = React.useState(null);
  const [activeList, setActiveList] = React.useState(null);
  const [rowStyle, setRowStyle] = React.useState(() => { try { return localStorage.getItem('antask_m_rowstyle') || 'tarjetas'; } catch (e) { return 'tarjetas'; } });
  const chooseRow = (id) => { setRowStyle(id); try { localStorage.setItem('antask_m_rowstyle', id); } catch (e) {} };
    ...
    <div style={{ height: '100%', position: 'relative', zIndex: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: t.canvas, color: t.ink, fontFamily: t.fontUI, '--tm': tw && tw.textScale != null ? tw.textScale : 1 }}>
      ...
      {isSub
        ? <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px 12px' }}>
            <span onClick={() => go(prev)} style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 16, fontWeight: 600, color: t.accentInk, padding: '6px 8px 6px 4px', cursor: 'pointer' }}><MIco name="chevronL" size={20} sw={2.2} />{titles[prev] || 'Atrás'}</span>
            <span style={{ flex: 1 }} />
            {screen === 'nota' && note && <span onClick={() => setNoteSheet(note.id)} aria-label="Opciones de la nota" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 38, height: 38, borderRadius: 11, cursor: 'pointer', color: t.ink2 }}><MIco name="ellipsis" size={21} /></span>}
            {screen === 'detalle' && task && <span onClick={() => setTaskSheet(task.id)} aria-label="Opciones de la tarea" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 38, height: 38, borderRadius: 11, cursor: 'pointer', color: t.ink2 }}><MIco name="ellipsis" size={21} /></span>}
          </div>
        : <MHeader t={t} title={titles[screen]} subtitle={subs[screen]} right={screen === 'hoy' ? hoyRight : null} />}

      {/* contenido */}
      <div ref={scrollRef} style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '0 16px 104px' }}>
