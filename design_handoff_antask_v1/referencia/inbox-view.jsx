...
const ANTASK_DATA = {
  ...
  bottom: [
  { icon: 'note', label: 'Notas', locked: true }],
...
/* ───────────────────────── ROW ───────────────────────── */
function TaskRow({ task, t, idx, onToggle, selected, onSelect, isMultiMode, isSelected, onToggleSelect, flash }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        position: 'relative', borderRadius: t.rowRadius,
        opacity: dragging ? 0.45 : 1,
        background: isSelected ? t.tintBg : selected ? t.tintBg : hover ? t.rowHover : elevated ? t.card : baseBg,
        border: elevated ? `1px solid ${isSelected || selected ? t.accent : t.border}` : 'none',
        boxShadow: flash ? `inset 0 0 0 2px ${t.accent}` : elevated ? hover ? '0 6px 16px -8px rgba(40,30,15,0.28)' : '0 1px 2px rgba(40,30,15,0.07)' : 'none',
        animation: flash && !window.__ANTASK_NOANIM ? 'qcFlash 1.6s ease-out' : 'none',
        marginBottom: elevated ? 7 : 0,
        borderBottom: !elevated && t.rowDivider ? `1px solid ${t.border}` : 'none',
        minHeight: Math.max((t.rowH || 42) + 9, 51),
        transition: 'background .12s, box-shadow .15s, opacity .15s'
      }}>
      {selected && <span style={{ position: 'absolute', left: 0, top: 4, bottom: 4, width: 3, borderRadius: 3, background: t.accent, fontWeight: "300", height: "25px", margin: "10px 0px 0px" }} />}
      {/* selección múltiple */}
      ...
      onClick={(e) => {e.stopPropagation();onToggle && onToggle(task.id);}} style={{ width: 28, height: 28, margin: '0 -4px', flexShrink: 0, display: 'grid', placeItems: 'center', cursor: 'pointer' }}>
        ...
        <span style={{ width: 19, height: 19, borderRadius: t.checkRadius, border: `1.7px solid ${hover ? t.accent : t.ink3}`, opacity: hover ? 0.9 : 0.55, transition: 'border-color .12s, opacity .12s' }} />}
      </span>
      <span style={{
        flex: 1, minWidth: 0, fontSize: 14.5, fontFamily: t.fontUI, fontWeight: 450,
        color: task.done ? t.ink3 : t.ink, letterSpacing: '-0.006em',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, opacity: task.done ? 0.55 : 1 }}>
        ...
        {task.list &&
        <span style={{ fontSize: 11, fontWeight: 600, fontFamily: t.monoMeta ? t.fontMono : t.fontUI, color: t.ink3, whiteSpace: 'nowrap', flexShrink: 0, opacity: 0.85 }}>{task.list}</span>}
...
function ProfileMenu({ t, sb, sbAccent, onOpenSettings }) {
  ...
  // Panel cálido elevado sobre la sidebar oscura.
  const panelBg = '#2b241a';
  /* Tintas propias del panel: SIEMPRE es una superficie cálida-oscura, así que no
     puede heredar sb.ink/ink2/border (en packs de sidebar clara como "papel
     editorial" son oscuros y el texto desaparecía sobre el panel oscuro). */
  const panelInk = '#f0e4d2';
  const panelInk2 = '#c4ad96';
  const panelInk3 = '#8a7864';
  const panelBorder = 'rgba(255,245,225,0.13)';
  const panelHover = 'rgba(255,235,200,0.10)';
  const panelAccent = sbAccent;
  const danger = '#e0846a';
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      style={{
        ...
        padding: '6px 8px', marginLeft: -8, marginRight: -8, marginTop: 4,
...
function DropMenu({ t, sb, surface = 'paper', align = 'right', placement = 'bottom', width = 212, header, sections = [], trigger }) {
  ...
  const S = surface === 'dark' ? {
    bg: '#2b241a',
    ink: '#f0e4d2', ink2: '#c4ad96', ink3: '#8a7864',
    border: 'rgba(255,245,225,0.13)',
    hover: 'rgba(255,235,200,0.10)', accent: sbx.accentInk || t.accentInk,
    danger: '#e0846a', dangerHover: 'rgba(224,132,106,0.14)',
    shadow: '0 1px 0 rgba(255,245,225,0.05) inset, 0 18px 44px -8px rgba(0,0,0,0.55), 0 4px 12px rgba(0,0,0,0.35)'
  } : {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, height: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
          <span style={{ position: 'relative', width: 16, height: 16, flexShrink: 0, display: 'grid', placeItems: 'center', animation: celebrate ? 'antaskRingPop .56s cubic-bezier(.34,1.4,.5,1)' : 'none' }}>
            {burst && <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `2px solid ${sbAccent}`, animation: 'antaskBurstRing .82s ease-out forwards', pointerEvents: 'none' }} />}
            <svg width="16" height="16" viewBox="0 0 16 16">
              <circle cx="8" cy="8" r="6" fill="none" stroke={sb.barTrack} strokeWidth="2" />
              <circle cx="8" cy="8" r="6" fill="none" stroke={sbAccent} strokeWidth="2" strokeLinecap="round"
        ...
        <span key={done} style={{ display: 'inline-block', fontSize: 11.5, fontWeight: 600, color: celebrate ? sbAccent : sb.ink3, fontVariantNumeric: 'tabular-nums', fontFamily: t.fontMono, animation: celebrate ? 'antaskCountPop .5s cubic-bezier(.34,1.4,.5,1)' : 'none', transition: 'color .3s' }}>{done}/{total}</span>}
      </div>
    </div>);
    ...
    <aside style={{ width: collapsed ? 62 : 226, flexShrink: 0, overflow: 'hidden', position: 'relative', zIndex: 40, background: sb.bg, fontFamily: t.fontUI,
      ...
      {collapsed ?
        ...
        <button type="button" title="Cuenta · miguel cantos" onClick={() => setCollapsed(false)}
          style={{ width: 34, height: 34, borderRadius: 10, background: t.tintBg, display: 'grid', placeItems: 'center', fontSize: 17, border: 'none', cursor: 'pointer', marginTop: 4 }}>🐼</button>
      </div> :
...
function DetailPanel({ t, task, onClose, onEdit, onToggle, onDelete }) {
  const floating = t.shell === 'floating';
  const PRIOS = [
  { key: null, label: 'Ninguna', color: t.ink3 },
  { key: 'baja', label: 'Baja', color: '#7c8a52' },
  { key: 'media', label: 'Media', color: '#c98a3c' },
  { key: 'alta', label: 'Alta', color: '#b0473f' }];

  const labelStyle = { fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: t.ink3, fontFamily: t.monoMeta ? t.fontMono : t.fontUI };
  const fieldWrap = { display: 'flex', flexDirection: 'column', gap: 9 };
  const inputStyle = { width: '100%', padding: '10px 12px', borderRadius: t.rInput, border: `1px solid ${t.border}`, background: t.inputBg, color: t.ink, fontSize: 14, fontFamily: t.fontUI, outline: 'none' };
  return (
    <aside style={{ width: '100%', flexShrink: 0, border: `1px solid ${t.border}`, borderLeft: floating ? `1px solid ${t.border}` : `1px solid ${t.border}`, borderRadius: floating ? t.rInput + 4 : 0, overflow: 'hidden', background: t.card, display: 'flex', flexDirection: 'column', fontFamily: t.fontUI, minHeight: 0, boxShadow: 'none', alignItems: "stretch", justifyContent: "flex-start", margin: 0, borderTop: floating ? undefined : 'none', borderRight: floating ? undefined : 'none', borderBottom: floating ? undefined : 'none' }}>
      {/* header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 18px', borderBottom: `1px solid ${t.border}`, flexShrink: 0, fontSize: "11px" }}>
        <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: t.ink3, fontFamily: t.monoMeta ? t.fontMono : t.fontUI }}>Detalle de tarea</span>
        <button type="button" onClick={onClose} aria-label="Cerrar panel de detalles" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, cursor: 'pointer', color: t.ink2, padding: 0, border: 'none', background: 'transparent', borderRadius: 6, transition: 'background .12s, color .12s' }} onMouseEnter={(e) => {e.currentTarget.style.background = t.rowHover;e.currentTarget.style.color = t.ink;}} onMouseLeave={(e) => {e.currentTarget.style.background = 'transparent';e.currentTarget.style.color = t.ink2;}}><Ico name="x" size={18} sw={2.2} /></button>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <span onClick={() => onToggle(task.id)} style={{ width: 21, height: 21, marginTop: 2, borderRadius: t.checkRadius, border: `1.8px solid ${t.ink3}`, flexShrink: 0, cursor: 'pointer' }} />}
          <textarea value={task.title} onChange={(e) => onEdit(task.id, { title: e.target.value })} rows={2}
          style={{ flex: 1, resize: 'none', border: 'none', outline: 'none', background: 'transparent', fontFamily: t.fontDisplay, fontWeight: 600, fontSize: 19, lineHeight: 1.32, color: task.done ? t.ink3 : t.ink, letterSpacing: '-0.01em', textDecoration: task.done ? 'line-through' : 'none' }} />
        </div>
        ...
        <div style={fieldWrap}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="date" value={task.due || ''} onChange={(e) => onEdit(task.id, { due: e.target.value || undefined })} style={{ ...inputStyle, colorScheme: t.mode, flex: 1 }} />
            <input type="time" value={task.time || ''} onChange={(e) => onEdit(task.id, { time: e.target.value || undefined })} style={{ ...inputStyle, colorScheme: t.mode, width: 116, flexShrink: 0 }} />
          </div>
        </div>
            ...
            {PRIOS.map((pr) => {
              ...
              return (
                ...
                style={{ flex: 1, textAlign: 'center', padding: '8px 0', borderRadius: t.rInput, fontSize: 12.5, fontWeight: 650, cursor: 'pointer',
                  border: `1px solid ${on ? pr.color : t.border}`, background: on ? hexA(pr.color, 0.13) : 'transparent', color: on ? pr.color : t.ink2, transition: 'background .12s' }}>{pr.label}</span>);
        ...
        <div style={fieldWrap}>
          ...
          <select value={task.list || 'Inbox'} onChange={(e) => onEdit(task.id, { list: e.target.value === 'Inbox' ? undefined : e.target.value })} style={{ ...inputStyle, cursor: 'pointer' }}>
            {DETAIL_LISTS.map((l) => <option key={l} value={l}>{l}</option>)}
          </select>
        </div>
      ...
      {/* footer */}
      <div style={{ padding: '13px 18px', borderTop: `1px solid ${t.border}`, display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flexShrink: 0 }}>
        <span onClick={() => onDelete(task.id)} style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', color: '#b0473f', fontSize: 13, fontWeight: 600 }}><Ico name="trash" size={15} />Eliminar</span>
      </div>
    </aside>);
...
function DetailRail({ t }) {
  ...
  return (
    <aside style={{ width: '100%', height: '100%', flexShrink: 0, overflow: 'hidden',
      ...
      style={{ width: 34, height: 34, display: 'grid', placeItems: 'center', borderRadius: 9, color: t.ink2 }}>
        <Ico name="panel" size={18} />
      </span>
      <div style={{ height: 1, width: 26, background: t.border, margin: '7px 0' }} />
      {RAIL_ICONS.map((ic) =>
      <span key={ic} style={{ width: 34, height: 34, display: 'grid', placeItems: 'center', borderRadius: 9, color: t.ink3, opacity: 0.55 }}>
        <Ico name={ic} size={17} />
      </span>)}
    </aside>);
...
{ value: 'cebra', label: 'Cebra', icon: 'layers' },
{ value: 'tarjetas', label: 'Tarjetas', icon: 'panel' }];
...
function InboxView({ theme: t, tw, setTweak, embed = false, hidden = false, archived = false }) {
  ...
  <main data-om-label="VISTA TAREAS" style={{ flex: 1, minWidth: 0, display: hidden ? 'none' : 'flex', flexDirection: 'column',
    ...
    overflow: floating ? 'hidden' : 'visible' }}>
        {/* header */}
        <div style={{ padding: '14px 26px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, fontFamily: t.fontMono, padding: '3px 9px', borderRadius: 999, background: t.tintBg, border: `1px solid ${t.accentInk}30`, color: t.accentInk, fontVariantNumeric: 'tabular-nums', lineHeight: 1.6, flexShrink: 0 }}>{counts[view]}</span>
          </div>
          <div style={{ display: 'flex', gap: 3, padding: '4px', background: t.segBg, borderRadius: t.rSeg + 2, border: `1px solid ${t.border}`, alignItems: 'center', flexShrink: 0 }}>
            {views.map((v) => {
          const on = v.id === view;
          return (
            <div key={v.l} onClick={() => setView(v.id)} style={{
              display: 'flex', alignItems: 'center', gap: 6,
              padding: '7px 14px', borderRadius: t.rSeg, cursor: 'pointer',
              background: on ? t.segOn : 'transparent',
              color: on ? t.accentInk : t.ink2,
              fontWeight: on ? 650 : 500, fontSize: 13,
              boxShadow: on ? t.segShadow !== 'none' ? t.segShadow : '0 1px 3px rgba(0,0,0,0.10), 0 0 0 1px rgba(0,0,0,0.06)' : 'none',
              transition: 'background .15s, color .15s, box-shadow .15s',
              userSelect: 'none'
            }}>
                  <Ico name={v.ic} size={14} sw={on ? 2.2 : 1.8} />
                  <span>{v.l}</span>
                </div>);
        })}
            {!archived &&
        <React.Fragment>
                <span style={{ width: 1, height: 18, background: t.border, margin: '0 2px', flexShrink: 0 }} />
                <RowStylePicker t={t} tw={tw} setTweak={setTweak} compact />
              </React.Fragment>
        }
          </div>
        </div>
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', padding: "16px 0 0" }}>
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
              <window.AgendaView theme={t} tasks={agendaTasks} selectedId={selectedAgendaId} onSelect={setSelectedAgendaId} onToggle={toggleAgenda} />
            </div>
          </div>}
        {view === 'mes' && window.MonthView &&
    <div style={{ flex: 1, minHeight: 0, display: 'flex', padding: "16px 0 0" }}>
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
              <window.MonthView theme={t} tasks={agendaTasks} selectedId={selectedMesId} onSelect={setSelectedMesId} onToggle={toggleAgenda} onGoAgenda={() => setView('agenda')} />
            </div>
          </div>}
      </main>;
...
function ListaBody({ t, tasks, toggle, selectedId, onSelect, hideAdd, onBulkDelete, onBulkComplete, flashId, onQuickAdd, externalFilter, onActiveList }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* filtros */}
      <div style={{ padding: '14px 26px 10px', maxWidth: 932, overflowX: 'auto', margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'nowrap', justifyContent: "flex-start" }}>
          ...
          {/* separador */}
          {dynamicFilters.length > 0 && <span style={{ width: 1, height: 16, background: t.border, flexShrink: 0 }} />}

          {/* chips: listas (con punto de color) */}
          <Chip id="Todas" label="Todas" />
          {!isFixedActive && activeFilter !== 'Todas' && !dynamicFilters.some((f) => f.id === activeFilter) &&
          <Chip id={activeFilter} label={activeFilter} />}
          {listFilters.map((f) => <Chip key={'l-' + f.id} id={f.id} label={f.label} dot={f.dot} />)}
      ...
      {/* list */}
      <div style={{ flex: 1, overflow: 'auto', padding: '2px 18px 96px', width: '100%' }}>
        <div style={{ maxWidth: 932, margin: '0 auto', width: '100%' }}>
        {(() => {
          const renderTask = (task) =>
          <TaskRow key={task.id} task={task} t={t} idx={filtered.indexOf(task)} onToggle={toggle} selected={task.id === selectedId} onSelect={onSelect} isMultiMode={isMultiMode} isSelected={selectedIds.has(task.id)} onToggleSelect={toggleSelect} flash={task.id === flashId} />;
          ...
          /* cabecera de grupo con barra de progreso en el color de la lista */
          const GroupHead = ({ name, dot, pendCount }) => {
            const st = name ? listStats[name] : null;
            const pct = st && st.total ? Math.round(st.done / st.total * 100) : 0;
            const col = dot || t.ink3;
            return (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 8px 8px' }}>
                <span style={{ width: 9, height: 9, borderRadius: '50%', background: col, boxShadow: `0 0 9px 0.5px ${col}`, flexShrink: 0 }} />
                <span style={{ fontFamily: t.fontTitle || t.fontUI, fontSize: 14.5, fontWeight: 700, letterSpacing: '-0.01em', color: t.ink }}>{name || 'Sin lista'}</span>
                <span style={{ fontFamily: t.fontMono, fontSize: 11, color: t.ink3 }}>{pendCount} pend.</span>
                {st && st.total > 0 &&
                <React.Fragment>
                    <span style={{ width: 86, height: 4, borderRadius: 999, background: t.border, overflow: 'hidden', flexShrink: 0 }}>
                      <span style={{ display: 'block', height: '100%', borderRadius: 999, width: pct + '%', background: col }} />
                    </span>
                    <span style={{ fontFamily: t.fontMono, fontSize: 10.5, color: t.ink3 }}>{st.done}/{st.total}</span>
                  </React.Fragment>
                }
          ...
          const pendBlock = (() => {
            if (!groupByList) return pend.map(renderTask);
            const byList = {};
          ...
          return (
            <React.Fragment>
              ...
              <div style={{ margin: '14px 8px 8px' }}>
                  ...
                  {showDone && <div style={{ marginTop: 4 }}>{done.map(renderTask)}</div>}
                </div>
              }
            </React.Fragment>);
        })()}
