// @ts-check
/** @type {Record<string, string>} */
export const en = {
  // ── Sidebar ───────────────────────────────────────────────
  "sidebar.search":           "Search tasks",
  "sidebar.search_short":     "Search",
  "sidebar.inbox":            "Inbox",
  "inbox.tasks_label":        "Tasks",
  "sidebar.today":            "Today",
  "sidebar.hide_sidebar":     "Hide sidebar",
  "sidebar.show_sidebar":     "Expand sidebar",
  "sidebar.pinned_section":   "Views",
  "sidebar.lists_section":    "Lists",
  "sidebar.add_list":         "Add list",
  "sidebar.list_name":        "List name",
  "pmenu.lists":              "Lists",

  // ── Profile ───────────────────────────────────────────────
  "profile.local":            "Local",
  "profile.local_storage":    "Local storage",
  "profile.edit_title":       "Your name",
  "profile.name_placeholder": "Type your name...",
  "profile.settings":         "Settings",
  "profile.export":           "Export workspace",
  "profile.import":           "Import workspace",
  "profile.shortcuts":        "Keyboard shortcuts",
  "profile.lang_switch":      "Cambiar a Español",
  "profile.synced":           "Synced",
  "profile.sync_active":      "Sync active",
  "profile.signout":          "Sign out",
  "profile.workspace_section":"Workspace",
  "profile.signin":           "Sync with Google",
  "profile.signin_email":     "Email link",
  "profile.signin_password":  "Email and password",
  "auth.signin_title":        "Sign in",
  "auth.signup_title":        "Create account",
  "auth.signin_btn":          "Sign in",
  "auth.signup_btn":          "Create account",
  "auth.email_ph":            "you@email.com",
  "auth.password_ph":         "Password",

  // ── Settings (two-pane dialog) ─────────────────────────────
  "settings.nav.account":              "Account",
  "settings.nav.appearance":           "Appearance",
  "settings.nav.notifications":        "Notifications",
  "settings.nav.tasks":                "Tasks",
  "settings.nav.data":                 "Data",
  "settings.nav.about":                "About",

  "settings.account.sync":             "Sync with Google",
  "settings.account.sync_desc":        "Your tasks are saved to the cloud.",
  "settings.account.connected":        "Connected",
  "settings.account.language":         "Language",
  "settings.account.language_desc":    "Interface language.",

  "settings.appearance.theme":         "Theme",
  "settings.appearance.theme_desc":    "Light for the day, dark for the night.",
  "settings.appearance.light":         "Light",
  "settings.appearance.dark":          "Dark",
  "settings.appearance.mode":          "Mode",
  "settings.appearance.mode_desc":     "Simple: only Today and Inbox on mobile. Full: every feature.",
  "settings.appearance.mode_simple":   "Simple",
  "settings.appearance.mode_full":     "Full",
  "settings.appearance.rowstyle_desc": "Clean: flat rows. Cards: with background and spacing.",
  "settings.appearance.accent":        "Accent color",
  "settings.appearance.accent_desc":   "Sets the character of the interface.",

  "settings.notifications.tasks":      "Task alerts",
  "settings.notifications.tasks_desc": "Reminders when a task is due.",
  "settings.notifications.times":      "Alert times",
  "settings.notifications.time":       "Alert time",
  "settings.notifications.add":        "Add",
  "settings.notifications.test":       "Test alert",
  "settings.notifications.digest":     "Daily digest",
  "settings.notifications.digest_desc":"An email every morning with the day's plan.",
  "settings.notifications.digest_soon":"Coming soon",

  "settings.tasks.monday":             "Week starts on Monday",
  "settings.tasks.monday_desc":        "Affects day order in dates.",
  "settings.tasks.default_priority":       "Default priority",
  "settings.tasks.default_priority_desc":  "Applied to new tasks without a priority.",
  "settings.tasks.priority_none":      "None",

  "settings.data.export":              "Export",
  "settings.data.export_desc":         "Download all your tasks as a file.",
  "settings.data.analytics":           "Anonymous analytics",
  "settings.data.analytics_desc":      "Cookie-free usage stats to improve the app. Your tasks are never sent.",
  "settings.data.import":              "Import",
  "settings.data.import_desc":         "Restore from a previously exported file.",
  "settings.data.clear":               "Clear",
  "settings.data.clear_completed":     "Clear completed tasks",
  "settings.data.clear_completed_desc":"Permanently deletes tasks that are already done.",
  "settings.data.delete_all":          "Delete everything",
  "settings.data.delete_all_desc":     "Erases every list and task. This can't be undone.",
  "settings.data.delete_all_btn":      "Delete everything",
  "settings.data.delete_all_confirm":  "You're about to permanently delete ALL your lists and tasks. This can't be undone. Are you sure you want to continue?",

  "settings.about.version":            "Version",
  "settings.about.version_desc":       "Do today. Repeat tomorrow.",
  "settings.about.session":            "Session",
  "settings.about.session_desc":       "Sign out on this device.",

  // ── Task input ────────────────────────────────────────────
  "task.input.placeholder":   "New task…",
  "task.add_btn":             "Add",
  "task.toggle_done":         "Mark as done",


  // ── Bottom nav (mobile) ───────────────────────────────────
  "nav.today":                "Today",
  "nav.calendar":             "Calendar",
  "nav.profile":              "Profile",

  // ── Toast / status ────────────────────────────────────────
  "toast.task_deleted":       "Task deleted",
  "toast.undo":               "Undo",
  "toast.task_recurred":      "↻ Task regenerated",

  // ── Defaults ─────────────────────────────────────────────
  "default.unnamed":          "Unnamed",

  // ── Quota modal ───────────────────────────────────────────
  "quota.title":              "Storage full",
  "quota.body":               "The browser rejected the save because available storage is full (~5 MB).<br><br><strong>Your latest changes were not saved.</strong><br><br>Export a backup and delete lists you no longer need.",
  "quota.export_btn":         "Export backup now",
  "quota.close_btn":          "Close",

  // ── Views ─────────────────────────────────────────────────
  "view.today_title":         "Today",

  // ── Accessibility ─────────────────────────────────────────
  "a11y.mobile_settings":     "Settings",
  "a11y.list_chips":         "Switch list",

  // ── Today (view) ──────────────────────────────────────────
  "today.empty_title_full":   "All caught up",
  "today.empty_sub_full":     "Nothing overdue and no tasks for today.",

  "hoy.overdue":              "Overdue",
  "hoy.for_today":            "For today",
  "hoy.nodate":               "No date · suggested",
  "hoy.move_all":             "Move all to today",
  "hoy.schedule_all":         "Schedule all for today",
  "hoy.move_one":             "Move to today",
  "hoy.schedule_one":         "Schedule today",
  "hoy.quickadd_ph":          "Add a task for today…",
  "header.empty":                "No tasks yet",
  "header.all_done":             "All done",
  "header.pending_one":          "{n} to do",
  "header.pending_other":        "{n} to do",
  "header.overdue_one":          "{n} overdue",
  "header.overdue_other":        "{n} overdue",
  "header.for_today":            "{n} for today",
  "header.next":                 "next: {date}",
  "header.left":                 "{n} left",
  "habits.today_of":             "{done} of {total} for today",
  "habits.none_today":           "None due today",
  "habits.best_streak_one":      "best streak {n} day",
  "habits.best_streak_other":    "best streak {n} days",
  "hoy.done_of":              "{done} of {total} done",
  "hoy.overdue_one":          "overdue",
  "hoy.overdue_other":        "overdue",
  "hoy.ring_title":           "Today's progress",
  "hoy.reopen":               "Reopen task",
  "hoy.cal_expand":           "Show the whole month",
  "hoy.cal_collapse":         "Show just the week",
  "hoy.cal_prev":             "Previous",
  "hoy.cal_next":             "Next",
  "hoy.back_to_today":        "Back to Today",
  "hoy.day_quickadd_ph":      "Add a task for this day…",
  "hoy.habits":               "Habits",
  "hoy.tab_tasks":            "Tasks",
  "hoy.habit_name_ph":        "Add a habit…",
  "hoy.habit_rename_prompt":  "Habit name",
  "hoy.habit_confirm_delete": "Delete the habit <strong>{name}</strong> and all its history?",
  "hoy.habit_freq_daily":     "Every day",
  "hoy.habit_freq_everyn":    "Every {n} days",
  "hoy.habit_freq_header":    "Frequency",
  "hoy.habit_freq_everyn_menu": "Every few days…",
  "hoy.habit_freq_prompt":    "Every how many days? (2 or more)",
  "hoy.habit_done_toggle":    "Mark done today",

  // ── Habits view ───────────────────────────────────────────
  "sidebar.habits":           "Habits",
  "habits.count_one":         "{count} habit",
  "habits.count_other":       "{count} habits",
  "habits.other_days":        "Other days",
  "habits.not_due":           "Not due today",

  // ── Historial de hábitos ──────────────────────────────────
  "hist.title":               "Habit history",
  "hist.open":                "View history",
  "hist.summary":             "{done} of {total} habits completed",
  "hist.pct":                 "{n}% completion",
  "hist.perfect":             "{n} perfect days",
  "hist.perfect_one":         "1 perfect day",
  "hist.range_label":         "Period",
  "hist.range_7":             "7 days",
  "hist.range_30":            "30 days",
  "hist.range_90":            "90 days",
  "hist.range_todo":          "All time",
  "hist.map_year":            "Past year",
  "hist.list_label":          "Habits",
  "hist.map_sr":              "Past-year map: {done} of {total} days fully completed.",
  "hist.less":                "Less",
  "hist.more":                "More",
  "hist.nothing_due":         "nothing due",
  "hist.empty":               "No habits to track yet.",
  "hist.streak":              "Current streak",
  "hist.per_habit":           "By habit",
  "hist.recent":              "Last {n} days",
  "hist.row_sr":              "{name}: {streak}-day streak, {done} of {due} completed in the period.",
  "capture.bar_hint":         "Press to type",
  "capture.open":             "Open quick capture",
  "inbox.group_none":         "No list",


  // ── Filters ───────────────────────────────────────────────
  "filter.trigger_label":     "Other",
  "filter.trigger_label_mobile": "Filters",
  "filter.all":               "All",
  "filter.pending":           "Pending",
  "filter.done":              "Done",
  "filter.overdue":           "Overdue",
  "filter.today":             "Today",
  "filter.nodate":            "No date",
  "filter.high":              "Important",

  // ── List ──────────────────────────────────────────────────
  "project.new_prompt":       "List name",
  "project.new_placeholder":  "my-list...",
  "project.rename":           "Rename list",
  "project.edit_this_list":   "Edit this list",
  "project.rename_prompt":    "Rename list",
  "project.delete":           "Delete list",
  "project.confirm_delete":   "Delete list <strong>{name}</strong> and all its tasks?",
  "project.change_color":     "Change color",
  "project.color_picker_title":"List color",
  "project.color_clear":      "No color",
  "sheet.new_list.title":     "New list",
  "sheet.new_list.color":     "Color",
  "sheet.new_list.confirm":   "Create list",
  "color.red":                "Red",
  "color.orange":             "Orange",
  "color.amber":              "Amber",
  "color.gold":               "Gold",
  "color.lime":               "Lime",
  "color.green":              "Green",
  "color.emerald":            "Emerald",
  "color.cyan":               "Cyan",
  "color.blue":               "Blue",
  "color.indigo":             "Indigo",
  "color.violet":             "Violet",
  "color.purple":             "Purple",
  "color.pink":               "Pink",
  "color.brown":              "Brown",
  "color.gray":               "Gray",
  "color.silver":             "Silver",

  // ── Task ──────────────────────────────────────────────────
  "task.rename_hint":         "Double-click to rename",
  "task.move_to_project":     "Move to another list…",
  "task.move_title":          "Move to another list",
  "task.in_list":             "List",

  // ── Task detail panel ─────────────────────────────────────
  "detail.title":             "Task details",
  "detail.close":             "Close details panel",
  "detail.rail_hint":         "Task detail — select a task to see its fields",
  "detail.title_placeholder": "Untitled",
  "detail.note":              "Note",
  "detail.note_placeholder":  "Add notes or details…",
  "detail.priority":          "Priority",
  "detail.priority_important": "Important",
  "detail.due_date":          "Date",
  "detail.no_date":           "No date",
  "detail.recur":             "Repeat",
  "detail.no_recur":          "Doesn't repeat",
  "detail.reminder":          "Reminder",
  "chip.reminder_label":      "Has a reminder",
  "chip.remove_important":   "Remove important",
  "chip.recur_label_one":     "Repeats every day",
  "chip.recur_label_other":   "Repeats every {n} days",
  "chip.go_to_list":          "Go to list {name}",
  "brote.fecha":              "Add date",
  "brote.fecha_corto":        "+ date",
  "brote.recur":              "Add repeat",
  "brote.importante":         "Mark as important",
  "sugerencia.aplicar":       "Suggested by the title: {label}. Click to apply",
  "sugerencia.descartar":     "Dismiss suggestion",
  "detail.no_reminder":       "No reminder",
  "detail.subtasks":          "Subtasks",
  "detail.subtask_placeholder": "Add subtask…",
  "detail.list":              "List",
  "detail.open":              "View details",

  // ── Subtask ───────────────────────────────────────────────
  "subtask.empty":            "No subtasks",
  "subtask.delete_aria":      "Delete subtask",

  // ── Common modals ─────────────────────────────────────────
  "modal.cancel":             "Cancel",
  "modal.save":               "Save",
  "modal.close":              "Close",
  "modal.accept":             "Accept",
  "modal.understood":         "Got it",
  "modal.clear":              "Clear",
  "modal.delete":             "Delete",
  "modal.confirm_title":      "Confirm",
  "modal.notice_title":       "Notice",
  "modal.error_title":        "Error",

  // ── Actions ───────────────────────────────────────────────
  "action.delete":            "Delete",
  "action.rename":            "Rename",
  "action.duplicate":         "Duplicate",

  // ── Date / date modal ─────────────────────────────────────
  "date.today":               "Today",
  "date.tomorrow":            "Tomorrow",
  "date.in_week":             "In 1 week",
  "date.yesterday":           "Yesterday",

  // ── Reminder ──────────────────────────────────────────────
  "reminder.preset.in_1h":    "In 1 hour",
  "reminder.preset.in_4h":    "In 4 hours",
  "reminder.preset.this_evening": "This evening (6 PM)",
  "reminder.preset.tomorrow_9am": "Tomorrow 9 AM",
  "reminder.preset.in_2_days":"In 2 days",

  // ── Recurrence ────────────────────────────────────────────
  "modal_recur.custom_placeholder": "Custom (days)",
  "modal_recur.custom_short":       "# days",
  "recur.daily":              "Daily",
  "recur.every_2_days":       "Every 2 days",
  "recur.weekly":             "Weekly",
  "recur.biweekly":           "Biweekly",
  "recur.monthly":            "Monthly",

  // ── Calendar ──────────────────────────────────────────────
  "calendar.month.1":  "January",   "calendar.month.2":  "February",
  "calendar.month.3":  "March",     "calendar.month.4":  "April",
  "calendar.month.5":  "May",       "calendar.month.6":  "June",
  "calendar.month.7":  "July",      "calendar.month.8":  "August",
  "calendar.month.9":  "September", "calendar.month.10": "October",
  "calendar.month.11": "November",  "calendar.month.12": "December",
  "calendar.day.mon":  "Mo", "calendar.day.tue": "Tu",
  "calendar.day.wed":  "We", "calendar.day.thu": "Th",
  "calendar.day.fri":  "Fr", "calendar.day.sat": "Sa",
  "calendar.day.sun":  "Su",
  "calendar.more":     "more",

  // ── Due badge (per task) ──────────────────────────────────

  // ── Search ────────────────────────────────────────────────
  "search.results_aria":      "Search results",
  "search.hint_nav":          "↑↓ navigate",
  "search.hint_open":         "↵ open",
  "search.placeholder":       "Search tasks…",
  "search.no_results":        "No results for",
  "search.count_one":         "1 result",
  "search.count_other":       "{n} results",

  // ── Quick capture ─────────────────────────────────────────
  "quick_capture.title":      "Quick capture",
  "quick_capture.placeholder":"e.g. Call the office tomorrow",
  "quick_capture.hint_type":          "Type",
  "quick_capture.hint_autocomplete":  "to autocomplete",
  "quick_capture.submit":     "Add task",
  "quick_capture.added_to":   "Added to",

  // ── Import / export ───────────────────────────────────────

  // ── Task preferences ──────────────────────────────────────
  "task_prefs.trigger":       "Details and columns",

  // ── Storage ───────────────────────────────────────────────

  // ── Notifications ─────────────────────────────────────────
  "notif.enable":             "Enable notifications",
  "notif.enabled":            "Notifications enabled",
  "notif.blocked":            "Notifications blocked",
  "notif.reminder_off":       "Notifications are off, so this reminder wouldn't reach you. Turn them on?",
  "notif.reminder_enable":    "Turn on",
  "notif.reminder_blocked":   "Your browser is blocking AnTrack's notifications. Allow them in the site settings to get this reminder.",
  "notif.reminder_unsupported": "This browser doesn't support notifications, so the reminder can't alert you.",
  "notif.unsupported":        "Notifications unsupported",
  "notif.error_min_time":     "At least one notification time must remain. Disable notifications above if you don't want them.",
  "notif.error_blocked":      "Notifications are blocked in this browser. Enable them in site settings to activate them here.",
  "notif.error_denied":       "Permission denied. Notifications cannot be shown.",
  "notif.error_not_enabled":  "Enable notifications first before testing them.",
  "notif.test_label":         "test",
  "notif.test_body":          "Notifications are working correctly.",
  "notif.task_overdue":       "Overdue task",
  "notif.task_due_today":     "Task due today",
  "notif.tasks_due":          "tasks due",
  "notif.digest_overdue":     "Overdue",
  "notif.digest_today":       "Today",
  "notif.reminder_title":     "Reminder",
  "notif.unknown_project":    "No list",
  "notif.remove_time":        "Remove time",

  // ── Sync ──────────────────────────────────────────────────
  "sync.conflict_title":      "Data conflict",
  "sync.use_cloud":           "☁ Use cloud data",
  "sync.use_local":           "💻 Upload my local data",

  // ── Keyboard shortcuts ────────────────────────────────────

  // ── Onboarding ────────────────────────────────────────────

  // ── Consent banner ────────────────────────────────────────
  "consent.text":             "AnTrack stores your tasks on your device (local storage). Do you also accept anonymous analytics to help us improve the app?",
  "consent.privacy_link":     "Privacy policy",
  "consent.accept":           "Accept analytics",
  "consent.decline":          "Essential only",

  // ── Backup / import ───────────────────────────────────────
  "backup.restore_title":     "Restore backup",
  "backup.restore_confirm":   "This will replace <strong>all your current lists</strong> with the ones in the backup. Continue?",
  "backup.restored_one":      "Backup restored with {count} list.",
  "backup.restored_other":    "Backup restored with {count} lists.",
  "backup.restored_with_secs":"Backup restored with {count} list(s) and {sec} section(s).",
  "backup.invalid_format":    "Invalid format. Make sure you import an AnTrack backup or task file.",
  "backup.need_active":       "Open a list before importing tasks.",
  "backup.add_tasks_one":     "<strong>1 task</strong> will be added to “{list}”. The tasks already in the list stay as they are.",
  "backup.add_tasks_other":   "<strong>{count} tasks</strong> will be added to “{list}”. The tasks already in the list stay as they are.",
  "backup.add_tasks_action":  "Add",
  "backup.parse_error":       "Could not import. Make sure the file is valid JSON.",

  // ── Today / Inbox menus ───────────────────────────────────
  "today.menu.complete_n":    "Complete the {count} tasks for today",
  "today.menu.complete":      "Complete today's tasks",
  "today.menu.complete_title":"Complete",
  "today.menu.complete_confirm":"Mark the <strong>{count}</strong> tasks for today as done?",
  "today.menu.postpone_all":  "Postpone all to tomorrow",
  "today.menu.postpone_title":"Postpone",
  "today.menu.postpone_confirm_one":   "Move <strong>{count}</strong> task to tomorrow?",
  "today.menu.postpone_confirm_other": "Move <strong>{count}</strong> tasks to tomorrow?",

  "inbox.menu.clear_done_n":  "Clear completed ({count})",
  "inbox.menu.clear_done":    "Clear completed",
  "inbox.menu.clear_done_title":"Clear",
  "inbox.menu.clear_done_confirm_one":   "Remove <strong>{count}</strong> completed task from the Inbox?",
  "inbox.menu.clear_done_confirm_other": "Remove <strong>{count}</strong> completed tasks from the Inbox?",
  "inbox.menu.empty":         "Empty Inbox",
  "inbox.menu.empty_title":   "Empty",
  "inbox.menu.empty_confirm": "Remove <strong>all {count} tasks</strong> from the Inbox? This cannot be undone.",

  // ── Empty states ──────────────────────────────────────────
  "empty.tasks.title_filtered":"No tasks for this filter.",
  "empty.tasks.title_new":    "No tasks yet in “{list}”.",
  "empty.tasks.sub_default":  "Create the first one for this list.",
  "empty.inbox.title":        "Inbox clear.",
  "empty.inbox.sub":          "All processed — capture something new when it comes up.",
  "empty.cta.add_task":       "New task",

  // ── Today badges (sidebar) ────────────────────────────────

  // ── Sync / profile ────────────────────────────────────────
  "profile.user_default":     "User",
  "project.kebab_title":      "Options",
  "project.dblclick_rename":  "Double-click to rename",
  "nav.tasks":                "List",
  "nav.calendar_short":       "Month",
  "filter.section_show":      "Show",
  "filter.section_when":      "When",
  "filter.section_attrs":     "Attributes",
  "theme.toggle":             "Toggle theme",
  "rowstyle.trigger":         "Row style",
  "rowstyle.limpio":          "Clean",
  "rowstyle.tarjetas":        "Cards",
  "list.search_placeholder":  "Search tasks…",
  "list.search_clear":        "Clear search",
  "task.no_other_projects":   "No other lists available.",
  "task.nothing_to_export":   "No lists to export.",
  "view.eyebrow_tasks":       "List",
  "view.eyebrow_calendar":    "Calendar",

  // ── Conflict / cloud-vs-local (body con placeholders) ─────
  "sync.conflict_body_one_one":     "You have <strong>{local} local list</strong> and <strong>{cloud} list in the cloud</strong>. Which do you want to use?",
  "sync.conflict_body_one_other":   "You have <strong>{local} local list</strong> and <strong>{cloud} lists in the cloud</strong>. Which do you want to use?",
  "sync.conflict_body_other_one":   "You have <strong>{local} local lists</strong> and <strong>{cloud} list in the cloud</strong>. Which do you want to use?",
  "sync.conflict_body_other_other": "You have <strong>{local} local lists</strong> and <strong>{cloud} lists in the cloud</strong>. Which do you want to use?",

  // ── Save status ───────────────────────────────────────────

  // ── List templates ────────────────────────────────────────
  "tpl.modal.title":          "New list",
  "tpl.modal.sub":            "Start from a template or a blank one",
  "tpl.blank.name":           "Blank",
  "tpl.blank.meta":           "Start from scratch",
  "tpl.meta.tasks_one":       "{count} task",
  "tpl.meta.tasks_other":     "{count} tasks",
  "tpl.preview.name_placeholder": "List name",
  "tpl.preview.tasks_count_one":   "Will create {count} task:",
  "tpl.preview.tasks_count_other": "Will create {count} tasks:",
  "tpl.preview.back":         "Back",
  "tpl.preview.confirm":      "Use template",
  "tpl.chip.today":           "Today",
  "tpl.chip.tomorrow":        "Tomorrow",
  "tpl.chip.in_days":         "In {n}d",
  "tpl.chip.recur":           "↻ Every {n}d",

  "tpl.moving.name":          "Moving",
  "tpl.moving.desc":          "Typical steps for a move",
  "tpl.moving.task.0":        "Compare moving companies",
  "tpl.moving.task.1":        "Book van or movers",
  "tpl.moving.task.2":        "Order boxes and packing supplies",
  "tpl.moving.task.3":        "Pack the kitchen",
  "tpl.moving.task.4":        "Pack the bedrooms",
  "tpl.moving.task.5":        "Pack the living room and rest",
  "tpl.moving.task.6":        "Notify address change (bank, subs)",
  "tpl.moving.task.7":        "Register at the new address",
  "tpl.moving.task.8":        "Moving day",
  "tpl.moving.task.9":        "Clean old place (hand over keys)",
  "tpl.moving.task.10":       "Set up power / water / internet at the new place",

  "tpl.trip.name":            "Trip",
  "tpl.trip.desc":            "Plan a trip with a flight",
  "tpl.trip.task.0":          "Book flights",
  "tpl.trip.task.1":          "Book accommodation",
  "tpl.trip.task.2":          "Check passport / ID expiry",
  "tpl.trip.task.3":          "Visas or entry requirements",
  "tpl.trip.task.4":          "Get local currency or fee-free card",
  "tpl.trip.task.5":          "Digital copy of important documents",
  "tpl.trip.task.6":          "Check weather forecast and pack",
  "tpl.trip.task.7":          "Online check-in and boarding pass",
  "tpl.trip.task.8":          "Check meds / chargers / adapters",

  "tpl.newjob.name":          "New job",
  "tpl.newjob.desc":          "First month at a new job",
  "tpl.newjob.task.0":        "Sign contract and submit paperwork",
  "tpl.newjob.task.1":        "Set up laptop and accounts (mail, slack…)",
  "tpl.newjob.task.2":        "Meet the team — 1:1 with each teammate",
  "tpl.newjob.task.3":        "Read internal product documentation",
  "tpl.newjob.task.4":        "Set up dev environment",
  "tpl.newjob.task.5":        "First meeting with manager (expectations)",
  "tpl.newjob.task.6":        "Write down questions and ask before Friday",
  "tpl.newjob.task.7":        "30-day review with manager",

  "tpl.course.name":          "Course / learning",
  "tpl.course.desc":          "Structure for a new course or topic",
  "tpl.course.task.0":        "Define a concrete learning goal",
  "tpl.course.task.1":        "Collect materials (books, videos, courses)",
  "tpl.course.task.2":        "Plan weekly sessions",
  "tpl.course.task.3":        "Do practice exercises",
  "tpl.course.task.4":        "Review what you learned",
  "tpl.course.task.5":        "Final mini-project applying everything",

  "tpl.event.name":           "Event or party",
  "tpl.event.desc":           "Organise a birthday, wedding, meet-up…",
  "tpl.event.task.0":         "Set date and venue",
  "tpl.event.task.1":         "Make guest list",
  "tpl.event.task.2":         "Send invitations (or save the date)",
  "tpl.event.task.3":         "Book catering or drinks",
  "tpl.event.task.4":         "Confirm RSVPs",
  "tpl.event.task.5":         "Decor and music",
  "tpl.event.task.6":         "Event day",
  "tpl.event.task.7":         "Thank attendees (photos, message)",

  "tpl.health.name":          "Medical errands",
  "tpl.health.desc":          "Appointments, tests and follow-up",
  "tpl.health.task.0":        "Book appointment with GP",
  "tpl.health.task.1":        "Bring list of meds and symptoms",
  "tpl.health.task.2":        "Tests / labs requested",
  "tpl.health.task.3":        "Pick up results",
  "tpl.health.task.4":        "Follow-up appointment",
  "tpl.health.task.5":        "Annual check-up",
};
